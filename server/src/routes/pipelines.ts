...
router.get('/:id', async (req: Request, res: Response) => {
  const [pipeline] = await db.select().from(pipelines).where(eq(pipelines.id, req.params.id));
  if (!pipeline) { res.status(404).json({ error: 'Pipeline not found' }); return; }
  res.json(pipeline);
});

router.put('/:id', async (req: Request, res: Response) => {
  const [updated] = await db.update(pipelines).set({ ...req.body, updatedAt: new Date() }).where(eq(pipelines.id, req.params.id)).returning();
  res.json(updated);
});
